...
// ADDED BY SHANE
	...
	{
if (DEBUG) System.out.println(" <ModulesFile_TidyUp>");
if (DEBUG && ! DEBUG_TU) System.out.println("  NOT showing the detailed debugging of tidyUp()");
		// Clear lists that will be generated by this method 
...
if (DEBUG_TU) System.out.println(" <FindAllFormulas>");
		findAllFormulas(formulaList);
if (DEBUG_TU) System.out.println(" </FindAllFormulas>");
if (DEBUG_TU) System.out.println("\ntidyUp(): 2 - About to call findCycles()\n");
if (DEBUG_TU) System.out.println(" <FindCycles>");
		formulaList.findCycles();
if (DEBUG_TU) System.out.println(" </FindCycles>");
if (DEBUG_TU) System.out.println("\ntidyUp(): 3 - About to call expandFormulas()\n");
if (DEBUG_TU) System.out.println(" <ExpandFormulas>");
		expandFormulas(formulaList);
if (DEBUG_TU) System.out.println(" </ExpandFormulas>");
		// Perform module renaming
if (DEBUG_TU) System.out.println("\ntidyUp(): 4 - About to call sortRenamings()\n");
if (DEBUG_TU) System.out.println(" <SortRenamings>");
		sortRenamings();
if (DEBUG_TU) System.out.println(" </SortRenamings>");

		// Check label identifiers
if (DEBUG_TU) System.out.println("\ntidyUp(): 5 - About to call checkLabelIdents()\n");
if (DEBUG_TU) System.out.println(" <CheckLabelIdents>");
		checkLabelIdents();
if (DEBUG_TU) System.out.println(" </CheckLabelIdents>");

		// Check module names
if (DEBUG_TU) System.out.println("\ntidyUp(): 6 - About to call checkModuleNames()\n");
if (DEBUG_TU) System.out.println(" <CheckModuleNames>");
		checkModuleNames();
if (DEBUG_TU) System.out.println(" </CheckModuleNames>");

		// Check constant identifiers
if (DEBUG_TU) System.out.println("\ntidyUp(): 7 - About to call checkConstantIdents()\n");
if (DEBUG_TU) System.out.println(" <CheckConstantIdents>");
		checkConstantIdents();
if (DEBUG_TU) System.out.println(" </CheckConstantIdents>");
		// Find all instances of constants
		// (i.e. locate identifiers which are constants)
if (DEBUG_TU) System.out.println(" <FindAllConstants>");
if (DEBUG_TU) System.out.println("\ntidyUp(): 8 - About to call findAllConstants()\n");
		findAllConstants(constantList);
if (DEBUG_TU) System.out.println(" </FindAllConstants>");
		// Check constants for cyclic dependencies
if (DEBUG_TU) System.out.println("\ntidyUp(): 9 - About to call constantList.findCycles()\n");
if (DEBUG_TU) System.out.println(" <FindCycles>");
...
if (DEBUG || DEBUG_TU) {
  System.out.println("\ntidyUp(): 10 - About to call convertIndexedDeclarations***()\n");
  System.out.println("<ConvIndSetDecls>");
		convertIndexedDeclarations(constantList,this);
  System.out.println("</ConvIndSetDecls>");
}	
		// Check variable names, etc.
if (DEBUG_TU) System.out.println("\ntidyUp(): 11 - About to call checkVarNames()\n");
if (DEBUG_TU) System.out.println(" <CheckVarNames>");
		checkVarNames();
if (DEBUG_TU) System.out.println(" </CheckVarNames>");
		// Find all instances of variables, replace identifiers with variables.
		// Also check variables valid, store indices, etc.
if (DEBUG_TU) System.out.println("\ntidyUp(): 12 - About to call findAllVars()\n");
//Update.DEBUG_MSG = true;
if (DEBUG_TU) System.out.println(" <FindAllVars>");
		findAllVars(varNames, varTypes);
if (DEBUG_TU) System.out.println(" </FindAllVars>");
//Update.DEBUG_MSG = false;

		// Find all instances of property refs
if (DEBUG_TU) System.out.println("\ntidyUp(): 13 - About to call findAllPropRefs()\n");
if (DEBUG_TU) System.out.println(" <FindAllPropRefs>");
		findAllPropRefs(this, null);
if (DEBUG_TU) System.out.println(" </FindAllPropRefs>");
		
		// Check reward structure names
if (DEBUG_TU) System.out.println("\ntidyUp(): 14 - About to call checkRewardsStructNames()\n");
if (DEBUG_TU) System.out.println(" <CheckRwdStrcNames>");
		checkRewardStructNames();
if (DEBUG_TU) System.out.println(" </CheckRwdStrcNames>");

		// Check "system...endsystem" constructs
if (DEBUG_TU) System.out.println("\ntidyUp(): 15 - About to call checkSystemDefns()\n");
if (DEBUG_TU) System.out.println(" <CheckSystemDefns>");
		checkSystemDefns();
if (DEBUG_TU) System.out.println(" </CheckSystemDefns>");
		
		// Get synchronising action names
		// (NB: Do this *after* checking for cycles in system defns above)
if (DEBUG_TU) System.out.println("\ntidyUp(): 16 - About to call getSynchNames()\n");
if (DEBUG_TU) System.out.println(" <GetSynchNames>");
		getSynchNames();
if (DEBUG_TU) System.out.println(" </GetSynchNames>");
		// Then identify/check any references to action names
if (DEBUG_TU) System.out.println("\ntidyUp(): 17 - About to call findAllActions()\n");
if (DEBUG_TU) System.out.println(" <FindAllActions>");
		findAllActions(synchs);
if (DEBUG_TU) System.out.println(" </FindAllActions>");

		// Various semantic checks 
if (DEBUG_TU) System.out.println("\ntidyUp(): 18 - About to call doSemanticChecks()\n");
if (DEBUG_TU) System.out.println(" <DoSemanticChecks>");
		doSemanticChecks();
if (DEBUG_TU) System.out.println(" </DoSemanticChecks>");
		// Type checking
if (DEBUG_TU) System.out.println("\ntidyUp(): 19 - About to call typeCheck()\n");
if (DEBUG_TU) System.out.println(" <TypeCheck>");
		typeCheck();
if (DEBUG_TU) System.out.println(" </TypeCheck>");

if (DEBUG_TU) System.out.println("\ntidyUp(): 20 - Basically completed the tidyUp() method");
...
if (DEBUG_TU) System.out.println("tidyUp(): 21 - Doing the extra thing...\n <SetUndefinedConstants>");
...
if (DEBUG_TU) System.out.println(" </SetUndefinedConstants>");
		}
if (DEBUG) System.out.println(" </ModulesFile_TidyUp>");
	}
			...
			for (Declaration d : varDecls)
{
   if (DEBUG_DeepCopy) {
	System.out.println("In ModulesFile.deepCopy(), dealing now with declaration: " + d + "[ " + d.getClass().getName() + " ]");
		...
		// Copy the names of any global-level indexed-sets.
