...
// SHANE does not think this is used - but it was in the source code that exists on official github repository at 4.4 of Prism.
...
throw new PrismLangException("Old-style Array Declarations should not be possible.");	// SHANE ADDED
	}
...
// ADDED BY SHANE
	...
	public void visitPost(Update e) throws PrismLangException
	{
		...
		for (i = 0; i < n; i++) {
			...
			isGlobal = isLocal ? false : (modulesFile.isGlobalVariable(var) || modulesFile.isGlobalIndexedSetVar(var));
			if (!isLocal && !isGlobal ) {
