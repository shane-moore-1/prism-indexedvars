...
package parser;		// SHANE believes that 'State' should be moved to the simulator package, rather than parser package, since it is a "runtime" thing of the simulator.
...
public class State implements Comparable<State>
{
	...
	public State(Values v, ModelInfo modelInfo) throws PrismLangException
	{
		...
		for (i = 0; i < n; i++) {
System.out.println("in parser.State constructor(Values,ModelInfo): Attempting to find VarIndex of: v.getName(i) for i=" + i +" which is " + v.getName(i)); System.out.flush();
			j = modelInfo.getVarIndex(v.getName(i));
			if (j == -1) {
//				throw new PrismLangException("Unknown variable " + v.getName(i) + " in state");
				PrismLangException ple = new PrismLangException("Unknown variable " + v.getName(i) + " in state");
ple.printStackTrace(System.out);
				throw ple;
			}
			if (varValues[i] != null) {
//				throw new PrismLangException("Duplicated variable " + v.getName(i) + " in state");
				PrismLangException ple2 = new PrismLangException("Duplicated variable " + v.getName(i) + " in state");
