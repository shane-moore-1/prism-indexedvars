...
public class PrismParser implements PrismParserConstants {
	 ...
	 */
        ...
        {
// SHANE has to insert 0, in order for the generated Parser to actually compile! But I don't see any reason why this method wants a parameter!
        ...
        protected PrismLangException generateSyntaxError(ParseException e)
        {
                ...
                if (e.currentToken == null) {
                        ...
                        if (msg != null && msg.length() > 0)
                                exMsg += ": " + msg;
e.printStackTrace();    //SHANE ADDED FOR DEBUGGING PURPOSES
...
//	Expression init = null;
        Token begin = null;
          begin = getToken(1); /*About to get identifier*/
    name = Identifier();
    switch ((jj_ntk==-1)?jj_ntk():jj_ntk) {
    case COLON:
      decl = NormalDecl(name);
      break;
    case LBRACKET:
      decl = IndSetDecl(name);
      break;
    default:
      jj_la1[18] = jj_gen;
      jj_consume_token(-1);
  ...
  static final public Declaration NormalDecl(String name) throws ParseException {
    ...
    declType = DeclarationVarType();
                          decl = new Declaration(name, declType); /*Non-Indexed Decl*/
    switch ((jj_ntk==-1)?jj_ntk():jj_ntk) {
    case INIT:
  ...
  static final public Declaration IndSetDecl(String name) throws ParseException {
        Declaration decl;
        DeclarationType declType;
        Expression init = null;
        Token begin = null;
    ...
    size = Expression(false,false);
    jj_consume_token(RBRACKET);
    jj_consume_token(COLON);
    declType = DeclarationVarType();
                            // NOTE: For arrays, the declType just obtained is the type for the elements. 
                            // The actual Declaration's type will be an IndexedSet. And during ModulesFile.tidyUp, 
  ...
  static final public Declaration Declaration2() throws ParseException {
        Declaration decl;
        String name = null;
        DeclarationType declType;
        Expression init = null;
...
// e.g. "a(b)" = "a" "(b)"
// Ignoring the warning results in the largest match being taken.
// SHANE has altered this to cater for allowing attempted accesses of elements in an indexed set (Reading only. See elsewhere for Updating.)
  static final public Expression ExpressionFuncOrIdent(boolean prop, boolean pathprop) throws ParseException {
        String s = null;
        Expression ret = null;
        Token begin = null;
    // If there is no brackets "(...)", this is an identifier
            s = Identifier();
    ...
    case LBRACKET:
      ...
      case LBRACKET:
        jj_consume_token(LBRACKET);
                          ret = new ExpressionIndexedSetAccess(s);
        ExpressionIndexedSetAccArgs(prop, pathprop, (ExpressionIndexedSetAccess)ret);
        jj_consume_token(RBRACKET);
        break;
      default:
        jj_la1[66] = jj_gen;
...
// SHANE HAS ADDED:
// The expression specifying which index to access in an indexed set access attempt (which ought to evaluate to an integer)
  static final public void ExpressionIndexedSetAccArgs(boolean prop, boolean pathprop, ExpressionIndexedSetAccess indSetAcc) throws ParseException {
        Expression expr;
    // I (SHANE) wonder if we can limit it to something like ExpressionTimesDivide, rather than mere Expression, to restrict the types of expressions we accept here.
            expr = Expression(prop, pathprop);
                                            indSetAcc.setIndexExpression(expr);
  }
...
// Identifier (returns String)
  static final public String Identifier() throws ParseException {
    jj_consume_token(REG_IDENT);
//		/*SHANE INSERTED LINE, for debugging: */ System.out.println("Matched an identifier: " + getToken(0).image); 
...
// Primed identifier - MODIFIED BY  SHANE
  static final public ExpressionIdent IdentifierPrime() throws ParseException {
        ...
        Token begin = null;
          begin = getToken(0);
    jj_consume_token(REG_IDENT);
                identName = getToken(0).image;                          // whatever was matched as the identifier's name.
//System.out.println("A possible primed Identifier was found: " + identName);
                ret = new ExpressionIdent(identName);
                ret.setPosition(token);
//		ret.setEndColumn(ret.getEndColumn() - 1);		// SHANE is unsure if this is correct now: is the minus-one needed still? Assuming not.
                                                                                                                // And would some different value be needed in the case of accessing an indexed set?

    switch ((jj_ntk==-1)?jj_ntk():jj_ntk) {
    case LBRACKET:
      jj_consume_token(LBRACKET);
                 ret = new ExpressionIndexedSetAccess(identName);               // Instead of ExpressionIdent, because it is more complex. 
                 ret.setPosition(token);                                                        // Copied from above; is it needed?
//	  	 ret.setEndColumn(ret.getEndColumn() - 1);		// Copied from above; is it needed?
                // IN THE FOLLOWING, I am not sure if the first two parameters are even required for the class; 
                // When ExprIndSetAccArgs is on an RHS, the values 'prop' and 'pathprop' are given, but the following
                // instance signifies an LHS of an update, which I don't think the parameters pertain; and maybe I 
                // should make a wholly different class for the following, but for now am putting 'falses' ...

      ExpressionIndexedSetAccArgs(false, false, (ExpressionIndexedSetAccess)ret);
      jj_consume_token(RBRACKET);
      break;
    default:
      jj_la1[102] = jj_gen;
    ...
    jj_consume_token(PRIME);
       ret.setPosition(begin, getToken(0));
//System.out.println("A primed Identifier was found: " + ret);
       {if (true) return ret;}
    throw new Error("Missing return statement in function");
  }
...
// For loop
  ...
  static {
   ...
   private static void jj_la1_init_0() {
      jj_la1_0 = new int[] {0x514404c0,0x504404c0,0x1000000,0xb01a0848,0x0,0xb01a0848,0xb01a0848,0x0,0xb01a0848,0x400,0x40000000,0x80,0x40000480,0x8000210,0x8000210,0x0,0x40,0x0,0x0,0x1000000,0x1000000,0x0,0x8000030,0x0,0x2000000,0x0,0x0,0x0,0xa0ba0808,0x0,0x0,0x0,0xa0ba0808,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0xa00000,0x0,0xa0ba0808,0xa0ba0808,0xa0ba0808,0xa0ba0808,0xa0ba0808,0x0,0x0,0x0,0x0,0x0,0x0,0xa01a0808,0x0,0x0,0x0,0x0,0x0,0x0,0xa01a0808,0xa01a0808,0x0,0x0,0xa0000000,0xa0000000,0x0,0x20000,0x0,0xa0000000,0x0,0x0,0x0,0xa0000000,0x0,0x0,0x0,0x0,0x0,0xa0000000,0x0,0x0,0xa0ba0808,0xa0ba0808,0x0,0x0,0xa4ba0908,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x1000000,0xa0000000,0x0,0xa0000000,0x0,0x0,0x0,0x0,};
   }
   private static void jj_la1_init_1() {
      jj_la1_1 = new int[] {0x187c5,0x83c4,0x10401,0x40127ab8,0x8000000,0x40127ab8,0x40127ab8,0x8000000,0x40127ab8,0x40,0x4,0x8000,0x8144,0x0,0x0,0x280,0x280,0x0,0x4000000,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x4012783a,0x200000,0x40020000,0x10000000,0x4012783a,0x0,0x0,0x10000000,0x0,0x10000000,0x10000000,0x0,0x40000000,0xc2000,0x0,0xc2000,0x2,0x0,0x4012783a,0x4012783a,0x4012783a,0x4012783a,0x4012783a,0x0,0x0,0x800000,0x1000000,0x400000,0x200000,0x40127838,0x0,0x0,0x0,0x0,0x0,0x0,0x40027838,0x40027838,0x40000000,0x40000000,0x0,0x0,0x10000000,0x20000,0x40000000,0x0,0x38,0x0,0x0,0x0,0x40000000,0x40000000,0x0,0x40000000,0x0,0x0,0x3800,0x0,0x4012783a,0x4012783a,0x0,0x4000,0x4012783a,0x0,0x3838,0x40003838,0x10000000,0x0,0x0,0x0,0x0,0x600000,0x10000000,0x0,0x0,0x0,0x0,0x4000000,};
   }
   private static void jj_la1_init_2() {
      jj_la1_2 = new int[] {0x0,0x0,0x0,0x1e08404,0x0,0x1e08404,0x1e08404,0x0,0x1e08404,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x40,0x1,0x0,0x0,0x19000001,0x1,0x1000000,0x0,0x1,0x1000000,0x4000,0x1e08404,0x0,0x0,0x0,0x1e08405,0x1000000,0x1,0x0,0x20010,0x0,0x0,0x20010,0x1200000,0x0,0x3341,0x0,0x0,0x3341,0x1e08404,0x1e08404,0x1e08404,0x1e08404,0x1e08404,0x3341,0x100000,0x0,0x0,0x0,0x0,0x1e08404,0xc0,0x3300,0xc000,0xc000,0x30000,0x30000,0x1e08404,0x1e00404,0x1,0x1,0x0,0x1000000,0x0,0xc00000,0x0,0x3340,0x0,0x10,0x10,0x0,0x0,0x3340,0x10,0x0,0x10,0x3340,0x0,0x10,0x1e08404,0x1e08404,0x20000,0x0,0x1e08404,0x404,0x0,0x0,0x0,0x1400000,0x10000,0x1400000,0x1000000,0x1004000,0x0,0x1000000,0x1,0xc0,0x3300,0x0,};
   }
