...
if (SHANE_DEBUG) {
	mainLog.println("in PrismMTBDD.Reachability() - after sanity check, about to call native PM_Reachability with the following parameters or using the following things received:");
	mainLog.println(" trans01 will be " + trans01.ptr() + ", which comes from trans01 which is this JDDNode: " + trans01);
	mainLog.println(" rv will be " + rows.array() + " which comes from rows which is this JDDVars: " + rows);
	mainLog.println(" nrv will be " + rows.n() + " which is from calling rows.n()");
	mainLog.println(" cv will be " + cols.array() + " which comes from cols which is this JDDVars: " + cols);
	mainLog.println(" ncv will be " + cols.n() + " which is from calling cols.n()");
	mainLog.println(" start will be " + start.ptr() + "which comes from start, which is this JDDNode: " + start);
	mainLog.println("the result will be a ptr, which gets wrapped up as a new JDDNode.");
	mainLog.println();
